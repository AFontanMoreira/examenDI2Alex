...
class Ventana(Gtk.Window):
    def __init__(self):
        ...
        CajaTV = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        ...
        self.lblAlabaran = Gtk.Label(label="Numero Albaran")
        ...
        dAlbaranes = bbdd.consultaSenParametros(
            "select * from main.detalleVentas where numeroAlbaran in (select numeroAlbaran from ventas)")
        print(nAlbaranes)
        print(dAlbaranes)
        ...
        CajaCmb.pack_start(self.lblAlabaran, False, False, 5)
    ...
    def on_cmbNumeroA_changed(self, combo, filtro, modelo):
        ...
        if seleccion is not None:
            ...
            print("Numero Selecconado: %d" % self.filtrado_albaran)
    ...
    def on_btnInforme_clicked(self, boton, bd):
        ...
        for venta in listaVenta:
            parrafo = Paragraph(
                "Albaran: " + str(venta[0]) + "    Fecha: " + str(venta[1]) + "     Cliente: " + str(venta[3]),
                cabecera_al)
            doc.append(parrafo)
